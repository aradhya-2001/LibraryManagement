...
@Component
public class UserPhoneFilterImpl implements UserFilterStrategy
{
    @Autowired
    ...
    public List<UserFilterResponse> getFilteredUser(Operator operator, String value)
    {
        ...
        users = switch (operator) {
            case EQUALS -> userRepository.findByPhoneNum(value);
            case LIKE -> userRepository.findByPhoneNumContains(value);
            default -> users;
        };
